...
> - `N/A — native app` — e.g. `Next.js App Router`, `Remix`, `SvelteKit`, `plain React`
> - `N/A — no web frontend` — e.g. `docs/sitemap.md`
> - `N/A — no API routes` — e.g. `routes/`, `src/handlers/`, `api/`
> - `N/A — native app` — e.g. `@next/bundle-analyzer`, `vite-bundle-visualizer`, `webpack-bundle-analyzer`
...
**Primary tool**: Instruments (Time Profiler, Allocations, Energy Diagnostics)
**Profile command**: `xcrun xctrace record --template "Time Profiler" --launch -- ./build/MyApp`
...
- Profiling tool: Instruments (Time Profiler, Allocations, Energy Diagnostics)
